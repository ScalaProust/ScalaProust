package scala.concurrent.stm.stamp.mapthroughputtest

import scala.annotation._
import scala.concurrent.stm.atomic
import scala.concurrent.stm.stamp.MapAdapterTrait

object MapThroughputTest extends scala.concurrent.stm.stamp.Benchmark[(Int, Int, Int, Int, Float)]{
	...
	override val appName: String = "java MapThroughputTest"
	...
	private def txnLoop(map:MapAdapterTrait[Int, Int], opCount:Int, opsPerTxn:Int, keyRange:Int, updatePct:Float):Unit = {
						...
						case (key, op) =>
							if(op < updatePct){
								if((op / updatePct) < 0.5){
									...
									map.put(key,random.nextInt(0, keyRange))
								}
							} else {
								map.get(key)
							}
	...
	protected def doBenchmark(params: ParamsType): Long = {
		...
		val map = ALLOCATOR.allocMap[Int,Int]
		val perThread = iterations / threadCount
		
		class BenchClass extends Thread {
			override def run:Unit = {
				barrier.await
				txnLoop(map, perThread, opsPerTxn, keyRange, updatePct)
				barrier.await
			}
		}
